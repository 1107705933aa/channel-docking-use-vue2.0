<template>
  <div id="access-provider">
    ...
    <!-- 接入商入驻 -->
    <el-row class="el-row-space" v-if="sys_cust_info_add">
      <el-col :span="24">
        <el-button
          class="padding-space"
          icon="el-icon-plus"
    ...
    <!-- 新增接入商入驻弹出框 -->
    <el-dialog title="新增接入商" :visible.sync="dialogAddFormVisible">
      <el-form
        :model="addForm"
        ref="addForm"
        :rules="rules"
      ...
      >
        <el-row>
          <el-col :span="11">
            <el-form-item label="接入商名称:" prop="custName">
              <el-input
                class="input-width"
                size="mini"
          ...
          <el-col :span="11" :push="2">
            <el-form-item label="接入商类型:" prop="custType">
              <el-input
                class="input-width"
                size="mini"
        ...
        <el-row v-if="hideData">
          <el-col :span="11">
            <el-form-item label="接入商负责人:" class="clear-space">
    ...
    >
      ...
      <el-table-column v-if="hideData" prop="custType" label="接入商类型" align="center"></el-table-column>
      <el-table-column prop="custTypeDesc" label="接入商类型" align="center"></el-table-column>
      <el-table-column v-if="hideData" prop="custStatus" label="接入商状态" align="center"></el-table-column>
      <el-table-column prop="custStatusDesc" label="接入商状态" align="center"></el-table-column>
      <el-table-column v-if="hideData" prop="personLiableId" label="接入商负责人Id" align="center"></el-table-column>
      <el-table-column prop="personLiableDesc" label="接入商负责人" align="center"></el-table-column>
      ...
      <el-table-column label="操作" width="245" align="center">
        <template slot-scope="scope">
          <el-button
            @click="handleJumpTerminal(scope.row)"
          ...
          <el-popover placement="top" width="190" v-model="scope.row.prohibitVisible">
            <p>
              ...
              <span class="popover-content">该行为将使接入商被禁用，是否继续？</span>
            </p>
    ...
    <el-dialog :title="editTitle" :visible.sync="dialogEditFormVisible">
      ...
      >
        <el-row>
          ...
          <el-col :span="11" :push="2">
            <el-form-item label="接入商类型:" prop="custType">
              <el-input class="input-width" size="mini" value="接入商" placeholder="请输入内容" disabled></el-input>
            </el-form-item>
          </el-col>
        ...
        <el-row class="top-space" v-if="hideData">
          <el-col :span="11">
            <el-form-item label="接入商负责人:" class="clear-space">
              <el-select v-model="editForm.personLiableIdValue" placeholder="请选择" size="mini">
                <el-option
    ...
    <!-- 审核接入商弹出框 -->
    <el-dialog title="审核接入商信息" :visible.sync="dialogCheckFormVisible">
      <el-form
        :model="checkForm"
        ref="checkForm"
        :rules="rules"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        custName: [
          { required: true, message: "请输入接入商名称", trigger: "blur" }
        ],
  ...
  created() {
    this.sys_cust_info_add = this.permissions["sys_cust_info_add"];
    this.sys_cust_info_edit = this.permissions["sys_cust_info_edit"];
    this.sys_cust_info_del = this.permissions["sys_cust_info_del"];
    this.sys_cust_info_audit = this.permissions["sys_cust_info_audit"];
    this.sys_cust_info_disable = this.permissions["sys_cust_info_disable"];
    if (this.sys_cust_info_add === undefined) {
      this.isActive = true;
    }
  },
  mounted() {
  ...
  methods: {
    ...
    // 跳转到终端管理
    handleJumpTerminal(row) {
      this.$router.push({
        path: "/link/business/access-logic/terminal/index",
        query: { id: row.custId }
      });
    },
    // 审核
